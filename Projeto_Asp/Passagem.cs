...
namespace Projeto_Asp
{
    public class Passagem
    {
        ...
        //método para update de dados
        public void updatePassagem(int id_passagem, int id_viagem, string cpf_cliente, double vlr_passagem, int poltrona)
        {
            SqlCommand comando = new SqlCommand();

            comando.Connection = conexao;

            comando.CommandText = "UPDATE passagem SET id_viagem=@id_viagem, cpf_cliente=@cpf_cliente, vlr_passagem=@vlr_passagem, poltrona=@poltrona WHERE id_passagem=@id_passagem";
